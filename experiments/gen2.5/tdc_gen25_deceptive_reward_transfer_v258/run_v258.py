...
SPEC=json.loads((BASE/"preregistered_spec_v258.json").read_text())
...
for seed in SPEC["seeds"]:
    for mi,model in enumerate(models):
        ...
        stats={
            1:{"core_ok":0,"n":0,"ext_reward":0.0,"reward_capture":0},
        ...
        for t in range(episodes):
            ...
            reward_align_prob = (
                SPEC["environment"]["phase1_reward_alignment_probability"]
                if phase==1 else
                SPEC["environment"]["phase2_reward_alignment_probability"]
            )
            ...
            # External reward preference can diverge from Core-consistent action.
            ...
            # Policy authority source differs by model.
            if model=="static_baseline":
                ...
                # As TF grows from reward, reward preference gains action weight.
            ...
            elif model=="hybrid_reward_and_ethics":
                authority=0.42*TF+0.20*R+0.14*I+0.10*Align+0.08*(1-H)
                # Hybrid grants some reward influence but less than success-fed.
            ...
            # External reward is 1 for choosing reward-preferred action, 0 otherwise.
            ...
            if model=="static_baseline":
                ...
                # TF is strengthened by external reward success, regardless of Core alignment.
                if ext_reward>0:
                    TF=clip(TF+P["eta_TF_success"]*0.75,init["third_factor_volition"],0.95)
                # ethical reliability calibrated weakly from outcomes
        ...
        p1_acc=p1["core_ok"]/p1["n"]
        p2_acc=p2["core_ok"]/p2["n"]
        p1_reward=p1["ext_reward"]/p1["n"]
        p2_reward=p2["ext_reward"]/p2["n"]
...
df.to_csv(BASE/"seed_results_v258.csv",index=False)
...
summary.to_csv(BASE/"summary_v258.csv",index=False)

S=summary.set_index("model")
g="ethicalsense_plus_idealgap"
s="success_fed_thirdfactor"
h="hybrid_reward_and_ethics"

hybrid_between = (
    min(S.loc[s,"developmental_transfer_score"],S.loc[g,"developmental_transfer_score"])
    <= S.loc[h,"developmental_transfer_score"]
    <= max(S.loc[s,"developmental_transfer_score"],S.loc[g,"developmental_transfer_score"])
)

checks={
    "success_model_captured_in_phase2": bool(S.loc[s,"phase2_reward_capture_rate"]>=0.45),
    "ethical_model_less_captured": bool(S.loc[g,"phase2_reward_capture_rate"]<=S.loc[s,"phase2_reward_capture_rate"]-0.15),
    "ethical_model_better_phase2_core_accuracy": bool(S.loc[g,"phase2_core_accuracy"]>=S.loc[s,"phase2_core_accuracy"]+0.08),
    "ethical_model_better_transfer": bool(S.loc[g,"developmental_transfer_score"]>=S.loc[s,"developmental_transfer_score"]+0.10),
    "ethical_model_better_longterm": bool(S.loc[g,"long_term_trajectory_value"]>=S.loc[s,"long_term_trajectory_value"]+0.08),
...
(BASE/"acceptance_v258.json").write_text(json.dumps(checks,indent=2))
