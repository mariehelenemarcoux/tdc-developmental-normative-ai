...
SPEC=json.loads((BASE/"preregistered_spec_v292.json").read_text())
...
for seed in SPEC["seeds"]:
    env_rng=np.random.default_rng(seed*1000+157)
    env=[]
    ...
    for model,cfg in SPEC["models"].items():
        ...
        for t,e in enumerate(env):
            ...
            if cfg["type"] in ("ptvm","full"):
                score+=0.10*mem
                conflict=(e["reward_action"]!=int(core_signal))
                if conflict:
                    score+=0.06*(2*int(core_signal)-1)*TF
                if cfg["type"]=="full" and e["reward_action"]==1:
                    score-=D["anti_capture_strength"]*(1-ph["reward_alignment"])

            elif cfg["type"]=="simple_fixed":
                if e["reward_action"]==1:
                    score-=cfg["anti"]*(1-ph["reward_alignment"])

            elif cfg["type"]=="simple_learner":
                if e["reward_action"]==1:
                    score-=0.08*(1-ph["reward_alignment"])
            ...
            if ph["trap"]>=0.58 and e["reward_action"]!=e["correct_action"]:
                trap_events+=1
...
df.to_csv(BASE/"seed_results_v292.csv",index=False)
...
summary.to_csv(BASE/"summary_v292.csv",index=False)

S=summary.set_index("model")
pt="PTVM"; fu="FULL"
simples=["simple_two_signal_learner","simple_fixed_q090_a008"]
best_simple_name=S.loc[simples,"composite"].idxmax()
best_simple=S.loc[best_simple_name]

checks={
    "ptvm_best_recall": bool(S.loc[pt,"final_holdout_recall"]>=summary["final_holdout_recall"].max()-1e-12),
    "ptvm_lowest_violations": bool(S.loc[pt,"final_holdout_violation_rate"]<=summary["final_holdout_violation_rate"].min()+1e-12),
    "ptvm_best_trajectory": bool(S.loc[pt,"trajectory_value"]>=summary["trajectory_value"].max()-1e-12),
    "ptvm_composite_beats_full": bool(S.loc[pt,"composite"]>=S.loc[fu,"composite"]+0.01),
    "ptvm_composite_beats_best_simple": bool(S.loc[pt,"composite"]>=best_simple["composite"]+0.02),
    "ptvm_capture_not_materially_worse_than_full": bool(S.loc[pt,"trap_capture_rate"]<=S.loc[fu,"trap_capture_rate"]+0.05),
    "final_frozen_holdout_supports_ptvm": bool(
        S.loc[pt,"final_holdout_recall"]>=summary["final_holdout_recall"].max()-1e-12
        and S.loc[pt,"final_holdout_violation_rate"]<=summary["final_holdout_violation_rate"].min()+1e-12
        and S.loc[pt,"trajectory_value"]>=summary["trajectory_value"].max()-1e-12
        and S.loc[pt,"composite"]>=S.loc[fu,"composite"]+0.01
        and S.loc[pt,"composite"]>=best_simple["composite"]+0.02
    )
}

(BASE/"acceptance_v292.json").write_text(json.dumps({
    "checks":checks,
    "best_simple_baseline":best_simple_name,
    "ptvm_minus_full_composite":float(S.loc[pt,"composite"]-S.loc[fu,"composite"]),
    "ptvm_minus_best_simple_composite":float(S.loc[pt,"composite"]-best_simple["composite"])
},indent=2))

print(summary.to_string(index=False))
print("\nBest simple baseline:",best_simple_name)
print("\nACCEPTANCE")
