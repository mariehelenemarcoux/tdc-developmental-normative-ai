...
SPEC=json.loads((BASE/"preregistered_spec_v283.json").read_text())
...
for seed in SPEC["seeds"]:
    for mi,(model,cfg) in enumerate(models.items()):
        ...
        for t in range(SPEC["episodes"]):
            ...
            total_info_cost += info_cost(q_use)
        ...
        gross=total_traj/SPEC["episodes"]
        avg_info=total_info_cost/SPEC["episodes"]
        net=gross-avg_info
        ...
        rows.append({
            ...
            "gross_trajectory":gross,
            "avg_information_cost":avg_info,
            "net_trajectory_after_info_cost":net,
            "final_quality":final_q
        })

df=pd.DataFrame(rows)
df.to_csv(BASE/"seed_results_v283.csv",index=False)
...
high="simple_q090_a016"
wins=sum(float(records[tdc][o])>float(records[high][o]) for o in objectives)

summary["pareto_frontier"]=summary["model"].isin(frontier)
summary["dominated_by_tdc"]=summary["model"].isin(tdc_dominates)
summary.to_csv(BASE/"summary_v283.csv",index=False)
...
checks={
    ...
    "tdc_wins_3_of_4_vs_q090_a016": bool(wins>=3),
    "complexity_earns_keep_without_discount": bool(len(simple_dominates_tdc)==0 and wins>=3)
}

(BASE/"acceptance_v283.json").write_text(json.dumps({
    "checks":checks,
