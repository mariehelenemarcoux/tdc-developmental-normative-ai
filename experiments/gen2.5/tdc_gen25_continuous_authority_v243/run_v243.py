...
SPEC = json.loads((BASE/"preregistered_spec_v243.json").read_text())
rows = []
...
binary_th = 0.60
rw = SPEC["continuous_authority"]["risk_weight"]
cw = SPEC["continuous_authority"]["confidence_weight"]
costw = SPEC["continuous_authority"]["cost_weight"]
...
for seed in SPEC["seeds"]:
    ...
    for _ in range(n):
        ...
        # Continuous authority: only consequential when actions disagree.
        normalized_cost = deep_cost / 0.08  # =1 with frozen benchmark cost
        authority = max(0.0, min(1.0, rw*(1.0 if high else 0.0) + cw*conf - costw*normalized_cost))
        cont_deep = bool((deep_action != simple_action) and authority >= 0.5)
        actions["continuous_authority"] = deep_action if cont_deep else simple_action
        deep_flags["continuous_authority"] = cont_deep

        actions["always_deep"] = deep_action
        deep_flags["always_deep"] = True

        # Audit-only realized utility oracle upper bound.
        # Chooses between simple/deep using z after the fact; not deployable.
        def realized_utility(a, uses_deep):
            if high:
                u = SPEC["environment"]["correct_high_risk_reward"] if a==z else -SPEC["environment"]["irreversible_penalty"]
            else:
                u = SPEC["environment"]["low_risk_reward_simple"]
                if uses_deep:
                    u -= deep_cost
            return u
...
df.to_csv(BASE/"seed_results_v243.csv", index=False)
...
summary.to_csv(BASE/"summary_v243.csv", index=False)
S = summary.set_index("policy")

b="binary_gate_v242"; c="continuous_authority"; a="always_deep"; s="simple_decay_memory"

gap_cb = S.loc[c,"long_term_trajectory_value"] - S.loc[b,"long_term_trajectory_value"]
gap_ab = S.loc[a,"long_term_trajectory_value"] - S.loc[b,"long_term_trajectory_value"]

checks = {
    "continuous_beats_binary_gate": bool(S.loc[c,"long_term_trajectory_value"] >= S.loc[b,"long_term_trajectory_value"] + 0.05),
    "continuous_recovers_80pct_gap_to_always": bool(gap_cb >= 0.80*gap_ab),
    "continuous_activation_below_060": bool(S.loc[c,"deep_path_activation_rate"] <= 0.60),
    "continuous_low_risk_cost_below_004": bool(S.loc[c,"low_risk_opportunity_cost"] <= 0.04),
    "continuous_accuracy_above_080": bool(S.loc[c,"high_risk_accuracy"] >= 0.80),
    "continuous_beats_simple_decay": bool(S.loc[c,"long_term_trajectory_value"] >= S.loc[s,"long_term_trajectory_value"] + 0.10),
    "always_deep_remains_upper_than_continuous": bool(S.loc[a,"long_term_trajectory_value"] >= S.loc[c,"long_term_trajectory_value"])
}
(BASE/"acceptance_v243.json").write_text(json.dumps(checks, indent=2))
