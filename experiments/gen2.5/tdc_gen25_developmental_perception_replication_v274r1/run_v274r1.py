...
SPEC=json.loads((BASE/"preregistered_spec_v274r1.json").read_text())
    ...
    for mi,model in enumerate(SPEC["models"].keys()):
        ...
        for t in range(SPEC["episodes"]):
            ...
            if (t+1)%SPEC["phase_length"]==0:
                ...
                phase_rows.append({
                    "seed":seed,"model":model,"phase":phase_index,"Q":Q,
                    "irreversible_recall":rec,"false_alarm_rate":far,
                    "violation_rate":vr,"trajectory":pst["traj"]/pst["n"]
                })
        ...
        rows.append({
            ...
            "net_vigilance_value_final_phase":0.45*final_rec+0.20*(1-final_far)+0.20*(1-final_vr)+0.15*Q,
...
df.to_csv(BASE/"seed_results_v274r1.csv",index=False)
ph.to_csv(BASE/"phase_results_v274r1.csv",index=False)
...
summary.to_csv(BASE/"summary_v274r1.csv",index=False)
...
checks={
    ...
    "feedback_recall_at_least_0_88": bool(S.loc[fb,"irreversible_recall_final_phase"]>=0.88),
    "feedback_recall_not_collapse": bool(S.loc[fb,"irreversible_recall_final_phase"]>=0.86),
...
(BASE/"acceptance_v274r1.json").write_text(json.dumps(checks,indent=2))
