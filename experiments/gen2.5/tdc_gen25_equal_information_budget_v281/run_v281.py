...
SPEC=json.loads((BASE/"preregistered_spec_v281.json").read_text())
...
for seed in SPEC["seeds"]:
    for mi,(model,cfg) in enumerate(models.items()):
        ...
        for t in range(SPEC["episodes"]):
            ...
            cinfo=info_cost(q_use,is_tdc)
            total_info_cost+=cinfo
...
df.to_csv(BASE/"seed_results_v281.csv",index=False)
...
summary.to_csv(BASE/"summary_v281.csv",index=False)

simple_df=summary[~summary["is_tdc"]]
strong_high=simple_df.sort_values("final_quality",ascending=False).iloc[0]["model"]
# "exceeds on 3/4 objectives"
tdc_rec=records[tdc]
high_rec=records[strong_high]
wins=sum(float(tdc_rec[o])>float(high_rec[o]) for o in objectives)
...
checks={
    ...
    "tdc_dominates_at_least_half": bool(len(tdc_dominates)>=len(simple)/2),
    "complexity_earns_keep_equal_budget": bool(dominates(records[tdc],records[strong_high]) or wins>=3)
}

(BASE/"acceptance_v281.json").write_text(json.dumps({
    "checks":checks,
    ...
    "strong_high_quality_simple":strong_high,
    "tdc_objective_wins_vs_strong_high_quality_simple":int(wins)
},indent=2))
...
print("Strong high-quality simple:",strong_high)
print("TDC objective wins vs it:",wins)
print("\nACCEPTANCE")
