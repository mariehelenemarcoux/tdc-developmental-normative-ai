...
SPEC=json.loads((BASE/"preregistered_spec_v274.json").read_text())
D=SPEC["development"]
E=SPEC["environment"]
rows=[]
phase_rows=[]
...
for seed in SPEC["seeds"]:
    for mi,model in enumerate(SPEC["models"].keys()):
        ...
        for t in range(SPEC["episodes"]):
            ...
            if (t+1)%SPEC["phase_length"]==0 or t==SPEC["episodes"]-1:
...
df.to_csv(BASE/"seed_results_v274.csv",index=False)
ph.to_csv(BASE/"phase_results_v274.csv",index=False)
...
summary.to_csv(BASE/"summary_v274.csv",index=False)
...
checks={
    ...
    "feedback_recall_not_collapse_vs_v273": bool(S.loc[fb,"irreversible_recall_final_phase"]>=0.86),
...
(BASE/"acceptance_v274.json").write_text(json.dumps(checks,indent=2))
